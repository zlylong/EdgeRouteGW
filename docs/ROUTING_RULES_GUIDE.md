...
- **Mode A**：支持以上三种。
- **Mode B / Mode C**：仅支持普通域名（如 `c.com`）；`*.` / `**.` 会被拒绝。

原因：Mode B/C 需要把规则转换为可下发的 OSPF 静态路由，wildcard 语义无法安全、确定地映射为稳定 CIDR。
...
- `priority` 越小，越先匹配。
- 若两条规则都能命中同一流量，以“更高优先级（更小 priority）”规则为准。
...
> 从 `v1.6.3` 开始，系统不再自动注入默认分流规则（如 `geosite:cn` / `category-ads-all` / `!cn`）。
> 新部署实例的规则列表默认是空的，完全由你按业务需求定义。

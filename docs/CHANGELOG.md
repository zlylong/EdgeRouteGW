...
- `install.sh`/`update.sh`：无法获取 `SHA256SUMS` 时拒绝安装（`PROXYGW_ALLOW_UNVERIFIED=1` 覆盖），不再内置固定回退版本号，不支持的架构直接报错；`update.sh` 保留旧二进制，重启后观察 10s（期间退出或被自动重启即判失败）并自动回滚；`systemctl restart` 失败不再因 `set -e` 跳过回滚。
- 脚本核对：`update.sh` 先下载校验再 `git reset`（失败不再留下半升级的工作树）、下载改用 curl（SOCKS 代理分支此前因 wget 不支持 `socks5h://` 必然失败）、reset 后检查 `core/xray/xray`、`core/mosdns/mosdns` 是否可运行并按架构重新下载（arm64 主机此前每次升级都会被还原成 amd64 二进制）、`aes.key` 恢复改为 EXIT trap、三张缓存表逐条清空；`install.sh` 先下载到 `.new` 再校验替换（失败不再留下空二进制）、初始 nftables 规则不再把网关自身出站打进 TPROXY（与后端模板一致，避免首启前自我黑洞）、`rt_tables` 改写入 `rt_tables.d/proxygw.conf`（Debian 13 无 `/etc/iproute2/rt_tables`）、补装 `sqlite3`、备份原 `resolv.conf`/`nftables.conf`、结束时校验 proxygw 确实在运行、版本优先取 GitHub API 且只取稳定标签、`$ID` 缺失不再触发 `set -u`；`xray.service` 增加 `ExecStartPre=mkdir -p /run/proxygw` 避免开机先于 proxygw 启动时以 23 退出；`build.sh` 强制 `CGO_ENABLED=1` 并检查 C 编译器、不再覆盖已配置的 `GOPROXY`；`db_optimize.sh` 校验 `DB_OPTIMIZE_BACKUP_KEEP>=1`、新库上跳过尚不存在的表；`flush_cache.sh` 只在原本运行时才重启服务并带 busy timeout；`test_frontend.sh` 用 `browser.js` 判断 Chromium 是否可用（`playwright install --dry-run` 恒为 0，旧判断每次都会尝试下载）；`pre-commit.sh` 用 `git rev-parse --show-toplevel` 定位仓库根（worktree 下原逻辑指向 `.git/worktrees`）；测试脚本把 `PROXYGW_HOME` 指向临时目录，避免包初始化在 `/root/proxygw/config` 生成 `aes.key`；`test_benchmark.sh` 加 `-v` 显示基准被跳过的原因；`uninstall.sh` 先停后端并清理 nft/ip 规则（含 IPv6）再停 xray/mosdns、还原备份的 `resolv.conf`/`nftables.conf`、清理 `proxygw.service.d` 与 `rt_tables.d`、无 systemd-resolved 的主机不再留下空链接、非交互 stdin 下不再中止。
